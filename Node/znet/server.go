...
func (s *Server) Start() {
	...
	go func() {                    // 防止start 函数等待连接阻塞
		...
		logrus.Infof("[start] Server %s Listenner at IP %s, Port %d is started", s.Name, s.IP, s.Port)
		// 3 阻塞，等待客户端连接，处理客户端连接业务，读写
		for {
			// 如果有客户端连接，此函数便有返回了，然后将conn传给我们自定的连接对象
			conn, err := listenner.AcceptTCP()
			if err != nil {
				logrus.Warnf("connection accept err: %v", err)
				continue
			}
			// 判断当前连接个数是否超过最大值，
...
func (s *Server) bindHeartBeatChecker(conn ziface.IConnection) {
	// 设定最大值和最小值，具体连接的发送间隔取其中的随机数。因为设定唯一值会使所有连接同时发心跳包，当连接过多时会导致突发流量
	randomSendInterval := GetRandomNumById(conn.GetConnID()) // 根据流ID 生成随机数。

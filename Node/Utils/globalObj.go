...
type GlobalObject struct {
	...
	LocalServerIp string         // 本终端的server的IP
	AllServerIps  []string       // 多模态项目所有的终端的server ip，每个client 都要与所有其他的server 相连
...
// 提供init方法 初始化对象
func init() {
	GlobalObj = &GlobalObject{ // 现在配置一些默认值，都只是默认值，在server或client中可以通过传入值来修改
		Name:          "millionTcpServer",
		LocalServerIp: "127.0.0.1", // 这只是默认值
		// 本机测试时只写 127 的地址
		// AllServerIps: []string{"127.0.0.1"},
		AllServerIps: []string{"10.162.162.200", "10.164.164.200", "10.166.166.200", "10.168.168.200", "10.172.172.200", "10.174.174.200"}, //},
		// AllServerIps: []string{"10.164.164.200", "10.166.166.200"},

		Port:               8990, // TCP 服务器端口
		ServerGinPort:      8991, // 服务器程序接收 文件传输命令 的服务器端口
		ClientGinPort:      8992, // 客户端程序接收 文件传输命令 的服务器端口
		Version:            "V1.0",
		MaxConn:            300000, // 单个server 最大 30w个
		MaxPackageSize:     1024,
		MaxFilePackageSize: 1 << 15, // 暂定32KB，本机器的tcp发送缓存大小为200KB；修改此处可以明显改变文件传输速度
		MinSendInterval:    600,     // 心跳包发送时间间隔设置
		MaxSendInterval:    1200,
		MinWaitTimt:        2, // 最小等待时间是直接加在下面两个值算出来的随机等待时间上的
		MeanWaitTimt:       30,
		MaxWaitTimt:        60,
		// FileDir:            "/home/sinet/multimodal/millionTcp/server/files/", // 本地测试的路径
		FileDir:   "/server/files/", // docker容器部署时的路径
		FileNames: []string{"v1_hpzg.mp4", "v2_hpzg.mp4", "v3_4k.mp4", "v4_4k.mp4"},
		FileSizes: []int64{5949948, 75313964, 124565867, 324563298},
	}

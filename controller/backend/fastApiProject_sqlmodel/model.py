...
class NetworkPerformance(SQLModel, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    protocol: str
    sharpness: Optional[float] = Field(default=None, description="清晰度")
    buffering: Optional[float] = Field(default=None, description="卡顿率")
    delay: Optional[float] = Field(default=None, description="时延")
    rate: Optional[float] = Field(default=None, description="感知速度")


class FirstOneBased(SQLModel):
    protocol: str


class FirstOne(FirstOneBased, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)


class FirstOneCreate(FirstOneBased):
    pass


class SecondOneBased(SQLModel):
    sharpness: Optional[float] = Field(default=None, description="清晰度")
    buffering: Optional[float] = Field(default=None, description="卡顿率")
    delay: Optional[float] = Field(default=None, description="时延")
    rate: Optional[float] = Field(default=None, description="感知速度")
...
class SecondOne(SecondOneBased, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)


class SecondOneCreate(SecondOneBased):
    pass

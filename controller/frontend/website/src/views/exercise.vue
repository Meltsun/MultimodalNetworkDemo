<template>
  ...
  <div class="tab-container">
    ...
    <el-table height="416" border  :data="backgroundTableData" stripe>
      <!-- "查看背景流"标签对应的表格列 -->
      ...
      <template slot="append">
        <div class="sum_footer">
          <div class="sum_footer_unit1 center" style="height: 30px;">
            合计
          </div>
          <div class="sum_footer_unit2">
            {{ sumActiveConnections }}
          </div>
          <div class="sum_footer_unit3">
            {{ sumSleepConnections }}
          </div>
          <div class="sum_footer_unit4">
            {{ sumTotalConnections }}
          </div>
        </div>
      </template>
    ...
    <div v-if="activeTab === 'protocols'" class="table-container">
      ...
      <el-table height="416" border :data="protocolsTableData"  stripe>
      <!-- "展示各协议组合标签"标签对应的表格列 -->
  ...
  <div v-if="activeTab === 'historyProtocols'" class="table-container">
    <div class="table-wrapper3">
      <el-table height="416" border :data="historyTableData" stripe>
      <!-- "展示各协议组合标签"标签对应的表格列 -->
      <el-table-column prop="state" label="阶段" width="75">
      <template slot-scope="scope">
        <div class="state"> {{ scope.row.state}}</div>
  ...
  computed: {
    // 计算总和，并添加逗号分隔符
    sumActiveConnections() {
      return this.formatNumber(this.calculateSum('active_connections', this.backgroundTableData));
    },
    sumSleepConnections() {
      return this.formatNumber(this.calculateSum('sleep_connections', this.backgroundTableData));
    },
    sumTotalConnections() {
      return this.formatNumber(this.calculateSum('total_connections', this.backgroundTableData));
    },
  },
  mounted() {
    this.fetchData();
  ...
  methods:{
    ...
    initChart() {
      ...
      this.$fetch(`link/${id}`)
        .then((linkInfo) => {
          ...
          const minDelay = Math.max(linkInfo1.delay, linkInfo2.delay);
          ...
          if (linkInfo1) {
            ...
            ${bulletPoint}  rate: ${sumRate2}Mbps<br>
            ${bulletPoint}  delay: ${minDelay2}ms<br>
            ${bulletPoint}  lost:${minLost}个
            `;
          }
        })
        .catch((error) => {
    ...
    this.$fetch('test/condition')
      ...
      if(selectedprotocol == "reno" || selectedprotocol == "bbr"){

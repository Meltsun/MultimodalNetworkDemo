...
control c_ingress(inout headers hdr, 
    ...
    apply {
        if (hdr.arp.isValid()) {
            ...
            hdr.probe_data[0].port_ingress = standard_metadata.ingress_port; // 存入入端口号
            ...
            last_time_ingress.read(temp_last_time_ingress, (bit<32>)standard_metadata.ingress_port) // 读取入端口上一个INT包进入时间
            ...
            last_time_ingress.write((bit<32>)standard_metadata.ingress_port, temp_last_time_ingress);  // 存入新上一个INT包进入时间
...
control c_egress(inout headers hdr, 
    ...
    apply {
        if (hdr.probe.isValid()) {
            // is the packet for int
            hdr.probe_data[0].port_egress = standard_metadata.egress_spec; // 存入出端口号
            ...
            last_time_egress.read(temp_last_time_egress, (bit<32>)standard_metadata.egress_spec) // 读取出端口上一个INT包进出时间
            ...
            last_time_egress.write((bit<32>)standard_metadata.egress_spec, temp_last_time_egress);  // 存入新上一个INT包进出时间
